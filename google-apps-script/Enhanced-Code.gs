// Enhanced Google Apps Script para manejar escritura detallada en Google Sheets
// Incluye TODAS las preguntas como columnas individuales para análisis detallado
...
function saveResponseToSheet(data) {
  try {
    ...
    if (lastRow === 0) {
      ...
      sheet.setColumnWidth(3, 200); // Universidad
      
      // Question columns
      for (let i = 9; i <= 8 + questionIds.length; i++) {
    ...
    // Aplicar formato condicional a las puntuaciones de preguntas (columnas 9 a 8+questionIds.length)
    const questionStartCol = 9;
    const questionEndCol = 8 + questionIds.length;
    
    if (questionStartCol <= questionEndCol) {
      const questionRange = sheet.getRange(newRowIndex, questionStartCol, 1, questionIds.length);
      
      // Formato condicional: 1-2 rojo, 3 amarillo, 4-5 verde
      const rules = sheet.getConditionalFormatRules();
      
      // Regla para puntuaciones bajas (1-2): rojo
      ...
      // Regla para puntuaciones medias (3): amarillo
      ...
      // Regla para puntuaciones altas (4-5): verde
    ...
    console.log(`Detailed data saved successfully to row ${newRowIndex} with ${fullRow.length} columns`);
...
function testDetailedSave() {
  const testData = {
    ...
    sessionId: `test_detailed_${Date.now()}`
  };
